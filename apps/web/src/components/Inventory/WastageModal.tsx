...
interface Props { item: InventoryItem & { status: any }; onClose: () => void; onSave: (qty: number) => void; }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm">
        ...
        <div className="px-5 py-4 border-t flex justify-end gap-3">
          ...
          <button onClick={() => onSave(+qty)} disabled={!qty || +qty <= 0 || +qty > item.currentQty} className="px-5 py-2 bg-red-600 text-white rounded-lg text-sm font-semibold hover:bg-red-700 disabled:opacity-50">Log Wastage</button>
        </div>
      </div>
    </div>
  );
